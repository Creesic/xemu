...
void pgraph_gl_draw_begin(NV2AState *d)
{
    ...
    uint32_t antialiasing = pgraph_reg_r(pg, NV_PGRAPH_ANTIALIASING);
    uint32_t draw_state_key[14] = {
        control_0, control_1, control_2, blend, blend_color, setup_raster,
        antialiasing, pg->surface_scale_factor, vp_width, vp_height, xmin, ymin,
        scissor_width, scissor_height,
    };

    if (!r->draw_state_valid ||
        memcmp(r->draw_state_key, draw_state_key,
               sizeof(draw_state_key)) != 0) {
        ...
        if (setup_raster & NV_PGRAPH_SETUPRASTER_CULLENABLE) {
            uint32_t cull_face = GET_MASK(
                setup_raster, NV_PGRAPH_SETUPRASTER_CULLCTRL);
        ...
        glFrontFace(setup_raster & NV_PGRAPH_SETUPRASTER_FRONTFACE
                        ? GL_CW : GL_CCW);
        ...
        if (stencil_test) {
            ...
            uint32_t stencil_func = GET_MASK(
                control_1, NV_PGRAPH_CONTROL_1_STENCIL_FUNC);
            uint32_t stencil_ref = GET_MASK(
                control_1, NV_PGRAPH_CONTROL_1_STENCIL_REF);
            uint32_t func_mask = GET_MASK(
                control_1, NV_PGRAPH_CONTROL_1_STENCIL_MASK_READ);
            uint32_t op_fail = GET_MASK(
                control_2, NV_PGRAPH_CONTROL_2_STENCIL_OP_FAIL);
            uint32_t op_zfail = GET_MASK(
                control_2, NV_PGRAPH_CONTROL_2_STENCIL_OP_ZFAIL);
            uint32_t op_zpass = GET_MASK(
                control_2, NV_PGRAPH_CONTROL_2_STENCIL_OP_ZPASS);
            ...
            glStencilFunc(pgraph_stencil_func_gl_map[stencil_func],
                          stencil_ref, func_mask);
            glStencilOp(pgraph_stencil_op_gl_map[op_fail],
        ...
        if (control_0 & NV_PGRAPH_CONTROL_0_DITHERENABLE) {
            glEnable(GL_DITHER);
        } else {
            glDisable(GL_DITHER);
        }
        ...
        bool anti_aliasing = GET_MASK(
            antialiasing, NV_PGRAPH_ANTIALIASING_ENABLE);
        if (!anti_aliasing &&
            setup_raster & NV_PGRAPH_SETUPRASTER_LINESMOOTHENABLE) {
        ...
        if (!anti_aliasing &&
            setup_raster & NV_PGRAPH_SETUPRASTER_POLYSMOOTHENABLE) {
        ...
        memcpy(r->draw_state_key, draw_state_key, sizeof(draw_state_key));
        r->draw_state_valid = true;
    }
