 ...
 * game/middleware code can compile against the shared Plume implementation.
 */
...
typedef enum D3DPRIMITIVETYPE {
    ...
    D3DPT_POINTLIST     = 1,
    D3DPT_LINELIST      = 2,
    D3DPT_LINELOOP      = 3,  /* Xbox-specific */
    D3DPT_LINESTRIP     = 4,
    D3DPT_TRIANGLELIST  = 5,
    D3DPT_TRIANGLESTRIP = 6,
    D3DPT_TRIANGLEFAN   = 7,
    D3DPT_QUADLIST      = 8,  /* Xbox-specific */
    D3DPT_QUADSTRIP     = 9,  /* Xbox-specific */
    D3DPT_POLYGON       = 10, /* Xbox-specific */
    D3DPT_MAX           = 11,
} D3DPRIMITIVETYPE;
...
typedef enum D3DTEXTURESTAGESTATETYPE {
    ...
    D3DTSS_ALPHAARG0    = 27,
    D3DTSS_RESULTARG    = 28,
    /* Internal normalized slot. Native D3D8 uses 24, which this frontend
     * already reserves for Xbox COLORKEYOP. */
    XRECOMP_D3DTSS_TEXTURETRANSFORMFLAGS = 31,
...
typedef enum D3DCLEAR_FLAGS {
    /* Xbox D3D8 exposes the NV097_CLEAR_SURFACE bit layout directly. */
    D3DCLEAR_ZBUFFER = 0x01,
    D3DCLEAR_STENCIL = 0x02,
    D3DCLEAR_TARGET_R = 0x10,
    D3DCLEAR_TARGET_G = 0x20,
    D3DCLEAR_TARGET_B = 0x40,
    D3DCLEAR_TARGET_A = 0x80,
    D3DCLEAR_TARGET = D3DCLEAR_TARGET_R | D3DCLEAR_TARGET_G |
                      D3DCLEAR_TARGET_B | D3DCLEAR_TARGET_A,
} D3DCLEAR_FLAGS;
...
/**
 * Set the host window title. Call before the device is created
 * (xbox_Direct3DCreate8 / CreateDevice). If not called, a generic
 * default is used. Native window creation remains host-owned.
 */
void xbox_d3d8_set_window_title(const char *title);

/**
 * Create the Xbox D3D8-compatible interface backed by Plume.
...
/** Present without blocking on vsync (FMV / skip polling). */
void d3d8_PresentFrameNoWait(void);

/** As PresentFrameNoWait, but from the PCRTC vblank fallback rather than a
 *  PGRAPH FLIP_STALL — kept distinct so the present trace can tell the two
 *  apart when diagnosing mid-frame presents. */
void d3d8_PresentFrameVblank(void);

/* Host-decoded full frame (RGBA8) → active renderer backbuffer → Present.
 * Used by branch movie players (nativeish/game/). Returns 1 on success. */
int d3d8_HostPresentFrameRGBA(const void *pixels, unsigned width,
                              unsigned height, unsigned row_pitch);

/**
 * Copy a linear CPU RGBA buffer into the swap-chain backbuffer.
 * Used by host-side boot/FMVs that already produce complete 640x480 frames.
 */
